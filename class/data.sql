...
WITH Names AS (
  SELECT unnest(ARRAY['Громозека', 'Ким', 'Буран', 'Зелёный', 'Горбовский', 'Ийон Тихий', 'Форд Префект', 'Комов', 'Каммерер', 'Ким']) AS name
), Ratings AS (
...
SELECT substring(md5(id::TEXT)::TEXT for 6), (0.5 + random() * pax)::INT, (0.5 + random() * flight)::INT
